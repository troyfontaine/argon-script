...
argon_ubuntu_setup() {
  raspi_ppa="ubuntu-raspi2"

  if ! grep -q "^deb .*$raspi_ppa" /etc/apt/sources.list /etc/apt/sources.list.d/*; then
      sudo add-apt-repository ppa:ubuntu-raspi2/ppa
      # Because the ubuntu-raspi2 PPA doesn't support focal, we have to tell it to use xenial
      sed -i 's/focal/xenial/g' /etc/apt/sources.list.d/ubuntu-raspi2-ubuntu-ppa-focal.list
  else
    echo "PPA ubuntu-raspi2 is already installed.  Skipping installation."
  fi

  archive_repo="archive.raspberrypi.org/debian"
  if ! grep -q "^deb .*$archive_repo" /etc/apt/sources.list; then
    ...
    echo "Repository $archive_repo is already installed.  Skipping installation"
  fi

  # Run apt update
  sudo apt-get update > /dev/null

  # Add shortcut for /usr/bin/python
  ...
  # We need this to get a compatible rpi.gpio version
  sudo apt-get install python3-pip

  # We use pip3 to install rpi.gpio as the apt package is out of date
  sudo pip3 install rpi.gpio
}

argon_distro=$(lsb_release -i -s)
argon_release=$(lsb_release -r -s)

if [ "$argon_distro" == "Ubuntu" ]; then
  
  if [ "$argon_release" == "20.04" ]; then
    # Ubuntu 20.04 does NOT come with Python2 by default
    argon_ubuntu_setup

    pkglist=(raspi-config libraspberrypi-bin python-rpi.gpio python3-smbus i2c-tools)
  else
		echo "********************************************************************"
		echo "Incompatible Ubuntu Release detected.  Exiting."
		echo "********************************************************************"
    exit
  fi

elif [ "$argon_distro" == "Raspbian" ]; then
